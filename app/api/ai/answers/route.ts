  ...
  try {
    ...
    const { text } = await generateText({
      ...
      **User's Answer:** ${userAnswer}
      ...
      system:
        "You are a helpful assistant that provides informative responses in markdown format. Use appropriate markdown syntax for headings, lists, code blocks, and emphasis where necessary. For code blocks, use short-form smaller case language identifiers (e.g., 'js' for JavaScript, 'py' for Python, 'ts' for TypeScript, 'html' for HTML, 'css' for CSS, etc.). For any code blocks, you MUST include a language identifier (e.g., ```js, ```py). Never use a code block without specifying a language. If you're unsure of the language, use ```text or ```plaintext.",
    });
